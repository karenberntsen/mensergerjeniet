...
public interface UserRepository extends CrudRepository<User, Long> {
    ...
    @Query("SELECT new  nl.MensErgerJeNiet.mensergerjeniet.db.model.services.exportobjects.UserInterface(u.id, u.userName, u.enabled) FROM User u ORDER BY u.userName")
    public List<UserInterface> findAll2();
    
    @Query("SELECT new  nl.MensErgerJeNiet.mensergerjeniet.db.model.services.exportobjects.UserInterface(u.id, u.userName, u.enabled) FROM User u WHERE u.enabled = ? ORDER BY u.userName")
